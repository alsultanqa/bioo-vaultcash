...
export default function Links(){
  ...
  async function load(page=1){ const j:Resp = await api(`/admin/payment-links?page=${page}&pageSize=${state.pageSize}${q?`&q=${encodeURIComponent(q)}`:''}`); setState(j); }
  useEffect(()=>{ load(1); }, []);
  return (<div>
    ...
    <table className="table"><thead><tr><th>Order</th><th>Amount</th><th>Status</th><th>Links</th></tr></thead>
      <tbody>{state.items.map(x=>(<tr key={x.token}>
      ...
      </tr>))}{state.items.length===0&&(<tr><td colSpan={4}>لا نتائج</td></tr>)}</tbody></table>
