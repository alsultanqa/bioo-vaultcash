...
export default function Links(){
  ...
  async function updateStatus(token:string, status:string){
    await api(`/admin/payment-links/${token}`, { method:'PATCH', body: JSON.stringify({ status }) });
    load(state.page);
  }
  async function remove(token:string){
    if(!confirm('حذف الرابط؟')) return;
    await api(`/admin/payment-links/${token}`, { method:'DELETE' });
    load(1);
  }
  ...
  return (
    <div>
      ...
      <table className="table">
        <thead><tr><th>Order</th><th>Amount</th><th>Status</th><th>Links</th><th>Actions</th></tr></thead>
        <tbody>
          {state.items.map(x=>(
            <tr key={x.token}>
              ...
              <td style={{display:'flex', gap:6}}>
                <select className="input" defaultValue={x.status} onChange={e=>updateStatus(x.token, e.target.value)}>
                ...
                <button className="btn" style={{background:'#ef4444'}} onClick={()=>remove(x.token)}>حذف</button>
              </td>
            </tr>
          ))}
          {state.items.length===0 && <tr><td colSpan={5}>لا نتائج</td></tr>}
        </tbody>
      </table>
