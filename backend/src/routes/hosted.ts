...
r.get('/:token', async (req, res) => {
  ...
  res.send(`
    <html><body style="font-family:system-ui">
      <h2>QatarCash • Checkout</h2>
      <p>Order: ${pl.orderId}</p>
      <p>Amount: ${amountQAR} ${pl.currency}</p>
      <form method="post" action="/webhooks/qatarchash/simulate-capture">
        <input type="hidden" name="paymentId" value="${pl.paymentId}" />
