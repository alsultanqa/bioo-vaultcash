...
export function signJwt(user: JwtUser){ return jwt.sign(user, SECRET, { expiresIn: '2h' }); }
export function requireAuth(req: Request, res: Response, next: NextFunction){
  const hdr = req.headers.authorization || ''; const token = hdr.startsWith('Bearer ') ? hdr.slice(7) : '';
  if(!token) return res.status(401).json({ error: 'unauthorized' });
  try{ (req as any).user = jwt.verify(token, SECRET) as JwtUser; next(); }catch{ return res.status(401).json({ error: 'unauthorized' }); }
}
